    ...
    public function render()
    {
        return <<<'blade'
            <div x-data="{ isOpen: false, openedWithKeyboard: false }" 
                ...
                <!-- Dropdown Menu -->
                <div x-cloak wire:key="dropdown-slot-{{ $uuid }}" x-show="isOpen || openedWithKeyboard" x-transition x-trap="openedWithKeyboard" x-on:click.outside="isOpen = false, openedWithKeyboard = false" x-on:keydown.down.prevent="$focus.wrap().next()" x-on:keydown.up.prevent="$focus.wrap().previous()" @class(["absolute flex w-fit min-w-48 flex-col overflow-hidden rounded-radius border border-outline bg-surface-alt dark:border-outline-dark dark:bg-surface-dark-alt z-20", $positionClass]) role="menu">
                    {{ $slot }}
                </div>
            </div>
